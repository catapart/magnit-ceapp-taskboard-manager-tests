...
\r
captioned-thumbnail::part(selected)\r
{\r
    display: none;\r
}\r
\r
...
\r
    ...
    task-list[collapsed]::part(header)\r
    {\r
        grid-template-columns: 1fr;\r
    }\r
    task-list[collapsed]::part(color-container)\r
    ,task-list[collapsed]::part(name)\r
    ,task-list[collapsed] .list-add-button-label\r
    {\r
...
{\r
    :host(:not([style*="color-scheme: light;"])) config-panel::part(key)\r
    {\r
        background-color: #444;\r
        color: #aaa;\r
        border-color: #666;\r
        box-shadow: 1px 1px rgba(255, 255, 255, 0.6),\r
        0 2px 0 0 rgba(255, 255, 255, 0.1) inset;\r
    }\r
}\r
:host([style*="color-scheme: dark;"]) config-panel::part(key)\r
{\r
    background-color: #444;\r
    color: #aaa;\r
    border-color: #666;\r
...
#tasklist-settings-handle:active\r
{\r
    cursor: grabbing;\r
} */\r
#tasklist-settings-handle\r
...
\r
#tasklist-settings-undo-icon\r
{\r
    display: none;\r
}\r
:host([removed]) #tasklist-settings-remove-icon\r
{\r
    display: none;\r
}\r
:host([removed]) #tasklist-settings-undo-icon\r
{\r
    display: flex;\r
}\r
\r
details\r
{\r
    --board-details-background: rgb(0 0 0 / .085);\r
    background-color: var(--board-details-background);\r
    border-radius: 3px;\r
    padding: 7px 3px;\r
}\r
\r
...
{\r
    ...
    border-width: 1px;\r
    border-color: rgb(170 170 170 / .75);\r
    background-color: transparent;\r
    border-radius: 3px;\r
}\r
#tasklist-color:hover\r
{\r
    border-color: rgb(170 170 170);\r
}\r
/* #tasklist-color::-webkit-color-swatch\r
{\r
    padding: 0;\r
    border: none;\r
}\r
#tasklist-color::-webkit-color-swatch-wrapper\r
{\r
    padding: 0;\r
    border: none;\r
} */\r
...
\r
#tasklist-settings-fieldset .fieldset-column\r
{\r
    display: grid;\r
    gap: .5em;\r
}\r
\r
#tasklist-settings-fieldset .column input[type="color"]\r
{\r
    height: auto;\r
}\r
\r
...
\r
#tasklist-width-field .container\r
{\r
    column-gap: .25em !important;\r
}\r
\r
#tasklist-width\r
,#tasklist-width-unit\r
{\r
    grid-row: 2;\r
}\r
\r
#tasklist-width-field [slot="postfix"]\r
{\r
...
{\r
    ...
    {\r
        --board-details-background: rgb(255 255 255 / .085);\r
    }\r
}`;
...
var app_menu_default = `#app-menu\r
{\r
    padding: 0;\r
    margin: 0;\r
    background-color: field;\r
...
var DataPanelElement = class extends HTMLElement {
  ...
  async init(options) {
    ...
    const daysToPersistData = await DataService.getAppSetting(
      "daysToPersistData"
      /* DaysToPersistData */
    ) ?? DEFAULT_PERSIST_DAYS;
  ...
  #applyDaysToPersist_onClick() {
    ...
    DataService.saveAppSetting("daysToPersistData", daysToPersist);
  }
  #clearData_onClick() {
    this.clearData();
  }
  ...
  #deletedItems_onRemove(event) {
    ...
    const targetType = recordType == "board" ? "board" : recordType == "list" ? "list" : recordType == "task" ? "task" : null;
...
var DataService = class _DataService {
  ...
  static async getAllBoardRecords() {
    const boardChannel = _DataService.#getChannel(
      _DataService.data.boards,
      "BOARD"
      /* BOARD */
    );
  ...
  static async getBoardRecords(...ids) {
    ...
    const channel = _DataService.#getChannel(
      _DataService.data.boards,
      "BOARD"
      /* BOARD */
    );
  ...
  static async createBoard(order) {
    ...
    const listChannel = this.#getChannel(
      this.#data.lists,
      "LIST"
      /* LIST */
    );
    const taskSettingsChannel = this.#getChannel(
      this.#data.taskSettings,
      "BOARD"
      /* BOARD */
    );
  ...
  static async createList(list, settings) {
    const taskLists = this.#getChannel(
      this.#data.lists,
      "LIST"
      /* LIST */
    );
  ...
  static async getTaskSettingsRecords(...ids) {
    ...
    const channel = _DataService.#getChannel(
      _DataService.data.taskSettings,
      "BOARD"
      /* BOARD */
    );
  ...
  static async getImageRecord(id) {
    const channel = _DataService.#getChannel(
      _DataService.data.customImages,
      "IMAGE"
      /* IMAGE */
    );
    return channel.get(id);
  ...
  static async reverseUpdate(channel, currentEntry, target) {
    ...
    if (currentEntry.data.properties.taskSettings != null && currentEntry.data.properties.taskSettings.updates != null) {
      const taskSettingsChannel = _DataService.#getChannel(
        _DataService.data.taskSettings,
        "SETTINGS"
        /* SETTINGS */
      );
    ...
    if (currentEntry.data.properties.backgroundImages != null) {
      const imagesChannel = _DataService.#getChannel(
        _DataService.data.customImages,
        "IMAGE"
        /* IMAGE */
      );
  ...
  static async getDeletedItems() {
    const boardChannel = this.#getChannel(
      this.#data.boards,
      "BOARD"
      /* BOARD */
    );
    ...
    const taskChannel = this.#getChannel(
      this.#data.tasks,
      "TASK"
      /* TASK */
    );
    const imageChannel = this.#getChannel(
  ...
  static async removeExpiredData() {
    ...
    const listChannel = _DataService.#getChannel(
      _DataService.#data.lists,
      "LIST"
      /* LIST */
    );
    const taskChannel = _DataService.#getChannel(
      _DataService.#data.tasks,
      "TASK"
      /* TASK */
    );
    ...
    const imageChannel = _DataService.#getChannel(
      _DataService.#data.customImages,
      "IMAGE"
      /* IMAGE */
    );
    const daysToPersistData = await _DataService.getAppSetting(
      "daysToPersistData"
      /* DaysToPersistData */
    ) ?? DEFAULT_PERSIST_DAYS;
  ...
  //#endregion Utilities
  //#region Internal
  static #getChannel(channel, errorType = "UNKNOWN") {
...
var WelcomePanelElement = class extends HTMLElement {
  ...
  async addBoardToRecentBoards(id, description, color) {
    ...
    DataService.saveAppSetting("recentBoards", boardsString);
  }
  async updateRecentBoardEntry(id, description, color) {
    const maxRecentBoards = await DataService.getAppSetting(
      "recentBoardsMax"
      /* RecentBoardsMax */
    ) ?? 10;
    const boards = await this.#getRecentBoards();
  ...
  async removeBoardFromRecentBoards(id) {
    ...
    const boardsString = JSON.stringify(boards);
    await DataService.saveAppSetting("recentBoards", boardsString);
  }
  async #getRecentBoards() {
    let boardsString = await DataService.getAppSetting(
      "recentBoards"
      /* RecentBoards */
    );
...
#board-settings-icon\r
{\r
    width: var(--dialog-header-icon-size);\r
    height: var(--dialog-header-icon-size);\r
}\r
\r
#board-settings-form\r
{\r
    display: flex;\r
    flex-direction: column;\r
    overflow: hidden;\r
...
#board-settings-cancel\r
{\r
    grid-column: 3;\r
}\r
#board-settings-save\r
{\r
    grid-column: 4;\r
}\r
\r
...
details\r
{\r
    --board-details-background: rgb(0 0 0 / .05);\r
    background-color: var(--board-details-background);\r
    border-radius: 3px;\r
    padding: 7px 3px;\r
}\r
...
.details-content\r
{\r
    padding: 14px 3px 3px 3px;\r
}\r
\r
form-field:not([id^="board-background-image-offset"]) input[inputmode="numeric"]\r
{\r
    width: 5ch;\r
}\r
\r
form-field:not([id^="board-background-image-offset"]) .container\r
{\r
    display: flex;\r
    flex-direction: column;\r
    gap: .25em;\r
...
\r
form-field .field-label .enabled-checkbox\r
{\r
    margin-right: 5px;\r
}\r
\r
...
/* #board-color::-webkit-color-swatch\r
{\r
    padding: 0;\r
    border: none;\r
}\r
#board-color::-webkit-color-swatch-wrapper\r
{\r
    padding: 0;\r
    border: none;\r
} */\r
\r
#board-name-field .container\r
{\r
    flex: 1;\r
}\r
\r
#board-appearance-fieldset .container > input\r
{\r
    flex: 1;\r
    min-height: 2ch;\r
}\r
\r
#board-background-image-display\r
{\r
    height: auto;\r
    flex: 1;\r
}\r
...
\r
#duplicate-fieldset .button\r
, #delete-fieldset .button\r
{\r
    justify-content: center;\r
}\r
\r
#duplicate-board-name-field .field-label\r
{\r
    margin-bottom: 7px;\r
}\r
\r
...
\r
form-field [part="container"]\r
{\r
    display: grid;\r
    grid-template-rows: auto 1fr;\r
    gap: .25em;\r
}\r
form-field [part="container"]:has([slot="postfix"])\r
{\r
    display: grid;\r
    grid-template-columns: 1fr auto;\r
    column-gap: 0;\r
...
form-field [part="container"]:has([slot="postfix"]) [part="field-label"]\r
{\r
    grid-column: span 2;\r
} */\r
\r
...
var SettingsPanelElement = class extends HTMLElement {
  ...
  #scheme_onChange;
  async init(options) {
    const scheme = await DataService.getAppSetting(
      "color-scheme"
      /* ColorScheme */
    );
...
var HistoryPanelElement = class extends HTMLElement {
  ...
  async init(options) {
    ...
    const historyLength = await DataService.getAppSetting(
      "historyLength"
      /* HistoryLength */
  ...
  async refresh() {
    ...
    let activeEntryIndex = await DataService.getAppSetting(
      "activeEntryIndex"
      /* ActiveEntryIndex */
    );
  ...
  async addActionHistoryEntry(action, type, properties) {
    const historyLength = parseFloat(await DataService.getAppSetting(
      "historyLength"
      /* HistoryLength */
    ) ?? DEFAULT_HISTORY_LENGTH);
    if (historyLength == 0) {
    ...
    await DataService.saveAppSetting("activeEntryIndex", activeIndex > -1 ? activeIndex : null);
    return entryElement;
  }
  async clearHistory() {
  ...
  async #applyHistoryLength(actionHistoryLength) {
    await DataService.saveAppSetting("historyLength", actionHistoryLength);
    const entries = await DataService.getHistoryEntries();
  ...
  async #handleActionEntryReverse(targetEntry, targetIndex) {
    ...
    await DataService.saveAppSetting("activeEntryIndex", targetIndex > -1 ? targetIndex : null);
  }
  async #handleActionEntryActivate(targetEntry, targetIndex) {
  ...
  async #actionHistory_onBack(target, previous, toReverse, targetIndex, previousActiveEntryIndex) {
  ...
  async #actionHistory_onForward(target, previous, toActivate, targetIndex, previousActiveEntryIndex) {
    let refreshBoards = false;
    for (let i = 0; i < toActivate.length; i++) {
...
var TaskboardManagerElement = class extends HTMLElement {
  ...
  */
  ...
  setColorScheme(scheme) {
    ...
    DataService.saveAppSetting("color-scheme", scheme);
  }
  async undo() {
  ...
  async addBoard() {
    ...
    await this.findElement("config-panel").addActionHistoryEntry(HistoryEntryType.Create, "board", { id: board.id });
    this.refreshBoardCollections();
  ...
  async removeBoard(boardId, confirm = true) {
    ...
    const entry = await configPanel.addActionHistoryEntry(HistoryEntryType.Delete, "board", { id: boardId });
    this.refreshBoardCollections();
  ...
  async #loadColorScheme() {
    const colorScheme = await DataService.getAppSetting(
      "color-scheme"
      /* ColorScheme */
    );
    if (colorScheme == null) {
  ...
  async #updateActionHistory(existingBoard, existingTaskLists, existingTaskSettings, board, taskLists, taskSettings, imageUpdates) {
    ...
    for (let i = 0; i < listActionProperties.length; i++) {
      ...
      await configPanel.addActionHistoryEntry(HistoryEntryType.Update, "list", actionProperties);
    }
    ...
    for (let i = 0; i < addedLists.length; i++) {
      ...
      await configPanel.addActionHistoryEntry(HistoryEntryType.Create, "list", { id: addedList.id });
    }
    ...
    for (let i = 0; i < removedLists.length; i++) {
      ...
      await configPanel.addActionHistoryEntry(HistoryEntryType.Delete, "list", { id: removedList.id });
    }
  }
  ...
  async #updateListRecord(taskListComponent) {
    ...
    await this.findElement("config-panel").addActionHistoryEntry(HistoryEntryType.Update, "list", properties);
  }
  async #updateTaskRecord(taskComponent, parentList) {
    ...
    await this.findElement("config-panel").addActionHistoryEntry(HistoryEntryType.Update, "task", properties);
  }
  ...
  async #addTaskRecord(boardId, listId, order) {
    ...
    task.order = order;
    await DataService.saveTaskRecords(task);
    this.findElement("config-panel").addActionHistoryEntry(HistoryEntryType.Create, "task", { id: task.id });
    return task;
  }
  async #deleteTaskRecord(taskComponent) {
    ...
    const entry = await this.findElement("config-panel").addActionHistoryEntry(HistoryEntryType.Delete, "task", { id });
    if (entry != null) {
  ...
  #applyTaskSettings(target, settings) {
    ...
    if (settings.colorDisplay == "hidden") {
    ...
    } else if (settings.colorDisplay == "border") {
    ...
    } else if (settings.colorDisplay == "top-border") {
      target.classList.remove("hide-task-color");
      target.classList.remove("task-color-background");
      target.classList.remove("color-border-right");
      target.classList.remove("color-border-bottom");
    ...
    } else if (settings.colorDisplay == "right-border") {
      target.classList.remove("hide-task-color");
      target.classList.remove("task-color-background");
      target.classList.remove("color-border-top");
      target.classList.remove("color-border-bottom");
    ...
    } else if (settings.colorDisplay == "bottom-border") {
      target.classList.remove("hide-task-color");
      target.classList.remove("task-color-background");
      target.classList.remove("color-border-top");
      target.classList.remove("color-border-right");
    ...
    } else if (settings.colorDisplay == "left-border") {
      target.classList.remove("hide-task-color");
      target.classList.remove("task-color-background");
      target.classList.remove("color-border-top");
      target.classList.remove("color-border-right");
    ...
    } else if (settings.colorDisplay == "background") {
      target.classList.remove("hide-task-color");
      target.classList.remove("task-color-border");
      target.classList.remove("color-border-top");
      target.classList.remove("color-border-right");
